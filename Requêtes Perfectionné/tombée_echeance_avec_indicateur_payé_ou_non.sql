-- ===========================================
-- REQUÊTE : Tombée d'échéance (Échéances du jour)
-- DESCRIPTION : Liste toutes les échéances du 05/03/2026 
--               avec indication si payée ou non
-- ===========================================
...
SELECT 
    ...
    -- === NOUVEAU : Statut enrichi avec indication de paiement ===
    -- CORRECTION : L'apostrophe dans "AUJOURD'HUI" est doublée pour l'échapper
    CASE 
        WHEN t.DATE_SOLDE IS NOT NULL THEN 'PAYÉ le ' + FORMAT(t.DATE_SOLDE, 'dd/MM/yyyy')
        WHEN t.DATE_ECHEANCE < CAST(GETDATE() AS DATE) THEN 'EN RETARD'
        WHEN t.DATE_ECHEANCE = CAST(GETDATE() AS DATE) THEN 'À PAYER AUJOURD''HUI'  -- Notez les deux apostrophes ''
        WHEN t.DATE_ECHEANCE BETWEEN CAST(GETDATE() AS DATE) AND DATEADD(day, 7, CAST(GETDATE() AS DATE)) THEN 'URGENT (7j)'
        ELSE 'À VENIR'
    END AS [Statut échéance],
    
    -- === INDICATEUR DE PAIEMENT (pour analyse) ===
    CASE 
        WHEN t.DATE_SOLDE IS NOT NULL THEN 1
        ELSE 0
    END AS [Est payé]
...
WHERE 1=1
    ...
    -- SUPPRIMÉ : le filtre AND t.DATE_SOLDE IS NULL
    -- Maintenant on inclut TOUTES les échéances (payées ou non)
...
ORDER BY 
    -- Tri par statut (les impayés en premier)
    CASE 
        WHEN t.DATE_SOLDE IS NULL AND t.DATE_ECHEANCE < CAST(GETDATE() AS DATE) THEN 1  -- En retard
        WHEN t.DATE_SOLDE IS NULL AND t.DATE_ECHEANCE = CAST(GETDATE() AS DATE) THEN 2  -- Aujourd'hui
        WHEN t.DATE_SOLDE IS NULL AND t.DATE_ECHEANCE <= DATEADD(day, 7, CAST(GETDATE() AS DATE)) THEN 3  -- Urgent
        WHEN t.DATE_SOLDE IS NOT NULL THEN 4  -- Payé
        ELSE 5  -- À venir
    END,
    t.DATE_ECHEANCE,
    ps.NOM,
